...
class TipoItemsController extends AppController {
		...
		if (!empty($this->data)) {
			...
			if ($this->TipoItem->validates()) {
				if ($this->TipoItem->save($this->data)) {
					$this->Log->write($this->Session->read('userdata.Usuario.usua_id'), utf8_encode('Nuevo Tipo de Ítem'), $_REQUEST);
					$this->Session->setFlash(__('El tipo de item ha sido guardado', true));
					$this->redirect(array('action' => 'index'));
				} else {
					$this->Session->setFlash(__(utf8_encode('El tipo de item no pudo ser guardado. Por favor inténtelo nuevamente'), true));
				}
			}
		}
	}
	...
	function edit($id = null) {
		...
		if (!empty($this->data)) {
			...
			if ($this->TipoItem->validates()) {
				if ($this->TipoItem->save($this->data)) {
					$this->Log->write($this->Session->read('userdata.Usuario.usua_id'), utf8_encode('Modificación Tipo de Ítem'), $_REQUEST);
					$this->Session->setFlash(__('El tipo de item ha sido guardado', true));
					$this->redirect(array('action' => 'index'));
				} else {
					$this->Session->setFlash(__(utf8_encode('El tipo de item no pudo ser guardado. Por favor inténtelo nuevamente'), true));
	...
	function delete($id = null) {
		...
		if ($this->TipoItem->delete($id)) {
			$this->Log->write($this->Session->read('userdata.Usuario.usua_id'), utf8_encode('Eliminación Tipo de Ítem'), $_REQUEST);
