...
//		}
		...
		if (parseInt(deaf_cantidad) >= 100) {		
			if (confirm('Aviso: Los bienes ingresados superan la cantidad de 100, ¿Desea continuar?') == 1) {
				errors = false;
			} else {
				showTipsy($("#deaf_cantidad"), 'Los bienes ingresados superan la cantidad de 100.');
				errors = true;
			}
		}
		...
		var html_str  = "<tr>";
			...
			html_str += "      <input id=\"mode_nombre\" type=\"hidden\" value=\""+ mode_nombre +"\" />";
			html_str += "      <input name=\"data[DetalleActivoFijo]["+ row_count +"][deaf_serie]\" id=\"deaf_serie\" type=\"hidden\" value=\""+ deaf_serie +"\" />";
			...
			html_str += "      <input name=\"data[DetalleActivoFijo]["+ row_count +"][deaf_fecha_adquisicion]\" id=\"deaf_fecha_adquisicion\" type=\"hidden\" value=\""+ deaf_fecha_adquisicion +"\" />";
			html_str += "   </td>";
			html_str += "</tr>";
	...
	var calculos_resumen = function() {
		...
		$.ajax({
			type: 'POST',
			url: '/activos_fijos/find_parametro_iva',
			dataType: 'json',
			cache: false,
			async: false,
			success: function(response) {
				valor_iva = response.valor;
				total_neto_iva = sum_valor_neto + (Math.round(parseInt(sum_valor_neto) * parseInt(valor_iva) / 100));
			}
		});
		
		$("#td_valor_neto").html("$"+ number_format(sum_valor_neto, 0, ",", "."));
		$("#td_iva").html("$"+ number_format(Math.round(parseInt(sum_valor_neto) * parseInt(valor_iva) / 100), 0, ",", "."));
		$("#td_total").html("$"+ number_format(Math.round(total_neto_iva), 0, ",", "."));
		$('#iva').html('').html('IVA ('+valor_iva+'%)');
	}
