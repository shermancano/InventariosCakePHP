		...
		$.ajax({
			...
			success: function(data) {
				$.each(data.info, function(i, obj) {
					var id = obj.prod_id+obj.deex_serie+obj.deex_fecha_vencimiento+obj.deex_precio;
					...
					if (!checkRepetido(id)) {
						...
						html_str += "   </td>";
						html_str += "   <td id=\"deex_total\">";
						html_str += "      "+ obj.total +" ";
						html_str += "   </td>";
						html_str += "   <td>";
						html_str += "      <input id=\"deex_cantidad\" maxlength=\"10\" name=\"data[DetalleExistencia]["+ row_count +"][deex_cantidad]\" value=\"0\" autocomplete=\"off\" onkeypress=\"return(validchars(event, num))\" />";
						html_str += "   </td>";
						html_str += "	<td>";
		...
		$("#table_detalle tr").each(function() {
			...
			if (!isNaN(deex_total)) {
				if (isNaN(deex_cantidad)) {
					showTipsy($(this).find("#deex_cantidad"), 'Debe ser numerico');
					errors = true;
				}
