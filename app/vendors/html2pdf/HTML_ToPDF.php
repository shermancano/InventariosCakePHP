<?php
/** $Id: HTML_ToPDF.php 2426 2006-11-18 19:59:26Z jrust $ */
// {{{ license
...
class HTML_ToPDF {
    ...
    /**
     * The full path to the html file or the html data
     * @var string
     */
    var $htmlFile = '';
     ...
     *
     * @param string $in_htmlFile The full path to the html file or the
    ...
    function setDebug($in_debug)
    {
        $this->debug = $in_debug;
        if ($this->debug) {
            $this->addHtml2PsSettings('option { debug: 1; }');
        }
    }
    ...
    function convert()
    {
        // make sure we have valid html or an html file
        if (preg_match('/<html.*<\/html>/ims', $this->htmlFile)) {
            $this->_htmlString = $this->htmlFile;
        }
        else {
        ...
        $this->_htmlString = preg_replace(':<input (.*?)type=["\']?(hidden|submit|button|image|reset|file)["\']?.*?>:i', '', $this->_htmlString);
