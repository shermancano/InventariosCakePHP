...
<h2><?php  __('Bajas de Activos Fijos');?></h2>
	<dl><?php $i = 0; $class = ' class="altrow"';?>
		...
		<dd<?php if ($i++ % 2 == 0) echo $class;?>>
			<?php echo $entrada['BajaActivoFijo']['baaf_id']; ?>
			&nbsp;
		</dd>
		<dt<?php if ($i % 2 == 0) echo $class;?>><?php __('Correlativo'); ?></dt>
		<dd<?php if ($i++ % 2 == 0) echo $class;?>>
			<?php echo sprintf("%012d", $entrada['BajaActivoFijo']['baaf_correlativo']); ?>
			&nbsp;
		</dd>
		...
		<dd<?php if ($i++ % 2 == 0) echo $class;?>>
			<?php echo date("d-m-Y H:i:s", strtotime($entrada['BajaActivoFijo']['baaf_fecha'])); ?>
			&nbsp;
		</dd>
        <dt<?php if ($i % 2 == 0) echo $class;?>><?php __('Número de Resolución'); ?></dt>
		<dd<?php if ($i++ % 2 == 0) echo $class;?>>
			<?php echo $entrada['BajaActivoFijo']['baaf_numero_documento']; ?>
			&nbsp;
		</dd>
        <dt<?php if ($i % 2 == 0) echo $class;?>><?php __('Observación'); ?></dt>
		<dd<?php if ($i++ % 2 == 0) echo $class;?>>
			<?php echo $entrada['BajaActivoFijo']['baaf_observacion']; ?>
			&nbsp;
		</dd>
	...
	<h2><?php  __('Detalle');?></h2>
        <table width="100%">
            ...
            ?>
                <tr>
                    <td><?php echo $det['DetalleBaja']['deba_codigo']; ?></td>
                    <td><?php echo $det['Producto']['prod_nombre']; ?></td>
                    <td><?php echo '$ '.number_format($det['DetalleBaja']['deba_valor_baja'], 0, ',', '.'); ?></td>                    
                </tr>
            <?php
                }
            ?>
